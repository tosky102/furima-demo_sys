 ...
 * **/
    ...
    private function __message_list($param) {
        $url = '/message/' . $this->action . '/?';
        $disp_num = 20;
        $pgnum = Hash::get($this->request->query, 'pg');
        if (empty($pgnum)) {
            $pgnum = 1;
        ...
        $this->set(compact('arrMessage', 'url'));
        $this->set('strPage', 'message');

    }
    function sendinglist(){
        $this->__message_list(array('from_member_id' => $this->user_id, 'status' => 1));
    }
    function receivedlist(){
        $this->__message_list(array('to_member_id' => $this->user_id, 'status' => 1));
    }
    function draftlist(){
        $this->__message_list(array('from_member_id' => $this->user_id, 'status' => 0));
    }
    function trashlist(){
        $this->__message_list(array('status' => 2));
    }
