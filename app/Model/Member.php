 ...
 * */
    ...
    */
     ...
     * 出品者ランキング
     */
    function getSellerRankEntity($limit = 5){
...
//        return $this->__getMembersByIdList($arrDatas);
    ...
    function getOfficialSellerEntity($limit = 5) {
        // SQL処理
        $sql = "";
        $sql .= "SELECT *";
        $sql .= "FROM `members` Member ";
        ...
        $sql .= "AND Member.role = 'user' ";
        $sql .= "AND Member.status = 1 ";
        $sql .= "AND Member.user_rank = 'official' ";
        $sql .= "GROUP BY Member.id  ";
//        $sql .= "ORDER BY MAX(Item.seller_rank) DESC ";
        $sql .= "ORDER BY Item.seller_rank ";
        $sql .= "LIMIT $limit ";
...
//        $arrRes["list"] = $this->__getMembersByIdList($array);
    ...
    function getOfficialSellerRankAll($disp_num,$pgnum){
        // ページング用パラメータ設定
        $this->setDispNum($disp_num);
        $this->setPgNum($pgnum);
        // SQL処理
...
//        $arrRes["list"] = $this->__getMembersByIdList($array);
    ...
    public function getDetail($id,$role = 'user') {
        ...
        $birthday = $arrDatas['Member']['birthday'];
        $ret = explode("-", $birthday);

        if (isset($ret[0]) && isset($ret[1]) && isset($ret[2])) {
            $arrDatas['Member']['birthday'] = array('year' => $ret[0], 'month' => $ret[1], 'day' => $ret[2]);
        } else {
            $arrDatas['Member']['birthday'] = array('year' => '', 'month' => '', 'day' => '');
        }
